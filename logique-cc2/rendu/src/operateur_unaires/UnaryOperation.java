...
import main.Model;
import main.World;
import operateur_binaires.BinaryOperation;
...
public abstract class UnaryOperation extends Formule {
    ...
    /** 
     * Getter sur la formule q
     * @return q Une formule
     */
    public Formule getQ() {
        return q;
    }
     ...
     * Cette méthode ajuste la représentation basée sur le type de l'opération contenue dans 'q'.
     * Si 'q' est une opération binaire, elle inclut la sous-formule dans des parenthèses pour clarifier
     * la précédence des opérations. Sinon, elle concatène simplement l'opérateur avec la représentation
     * de 'q'.
     * 
     * @return Une chaîne de caractères représentant l'opération avec l'opérateur suivi de la formule 'q'.
     */
    @Override
    public String toString() {
        if (this.q instanceof BinaryOperation) {
            return this.operateur + "("+this.q.toString()+")";
        }
        return this.operateur + this.q.toString();
    }
